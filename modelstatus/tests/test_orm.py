...
class TestOrm(unittest.TestCase):
    ...
    def test_uniq_constraint(self):
        ...
        reference_time = datetime.datetime.strptime(date_str, 
                                                    "%Y-%m-%d %H:%M:%SZ")
        provider = "arome25"
    ...
    def test_db_lookup(self):
        ...
        self.assertEqual(1, 
                         self.session.query(modelstatus.orm.ModelRun).count())

        mr = self.session.query(modelstatus.orm.ModelRun).first()
        self.assertEqual(mr.reference_time.strftime("%Y-%m-%d %H:%M:%SZ"), 
                         date_str)
    ...
    def test_get_max_version(self):
        ...
        res = self.session.query(modelstatus.orm.ModelRun).filter(
            modelstatus.orm.ModelRun.data_provider == provider,
            modelstatus.orm.ModelRun.reference_time == reference_time,
            
            ).order_by(modelstatus.orm.ModelRun.id.desc()).first()

        self.assertEqual(3, res.version)    
